...
const Header = () => {
 ...
 const [isMenu,setIsMenu]= useState(); //click on profile pic and get menu if true 
  ...
  // update user actionType and details
    dispatch(
      {
    ...
    localStorage.setItem("user",JSON.stringify(providerData[0]));
      ...
      //if user is already logged in then onclick show menu
      setIsMenu(!isMenu);
    }
  ...
  return (
    <header className='fixed z-50 w-scree p-3 px-4 md:p-6 md:px-16'>
        {/* desktop and tablet */}
       ...
       <div className='flex  items-center ml-320 gap-7'>
         {/* menu options  */}
        <motion.ul 
        ...
        className='flex gap-8 items-center' >
            <li className='textbase text-textColor hover:text-headingColor duration-100
     ...
     <div className='relative'>      
         < motion.img 
         whileTap={{scale:0.7,opacity: 0.8}}
         src={user ?user.photoURL :Avatar } alt="avatar"
         className=' w-8 h-8 min-w-[40] min-h-[40] cursor-pointer rounded-full'
         onClick={login} />

          {/* render menu only if isMenu is true */}
         {
           isMenu && (
            <motion.div 
            ...
            {
              user && user.email==='mauryaravikant555@gmail.com' && (
                <Link to={'/createitem'}>
